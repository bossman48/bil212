...
public class TreeMap<K,V> extends AbstractSortedMap<K,V> {
  ...
  protected static class BalanceableBinaryTree<K,V>
    ...
    public Position<Entry<K,V>> rotateR(Position<Entry<K,V>> p){
    	Node<Entry<K,V>> z=validate(p);
    	Node<Entry<K,V>> y=z.getLeft();
    	Node<Entry<K,V>> x=z.getParent();
    ...
    public Position<Entry<K,V>> rotateL(Position<Entry<K,V>> p){
    	Node<Entry<K,V>> z=validate(p);
    	Node<Entry<K,V>> y=z.getRight();
    	Node<Entry<K,V>> x=z.getParent();
    ...
    public Position<Entry<K,V>> rotateLR(Position<Entry<K,V>> p){
    	Node<Entry<K,V>> z=validate(p);
    	Node<Entry<K,V>> x=z.getParent();
    	Node<Entry<K,V>> w=z.getLeft();
    	rotateL(w);
    	Position<Entry<K,V>> y=rotateR(z);
    	return y;
    }
    
    public Position<Entry<K,V>> rotateRL(Position<Entry<K,V>> p){
    	Node<Entry<K,V>> z=validate(p);
    	Node<Entry<K,V>> x=z.getParent();
    	Node<Entry<K,V>> w=z.getRight();
    	rotateR(w);
    	Position<Entry<K,V>> y=rotateL(z);
    	return y;
    }
  ...
  protected Position<Entry<K,V>> rotateR(Position<Entry<K,V>> p) { return tree.rotateR(p); }
  protected Position<Entry<K,V>> rotateL(Position<Entry<K,V>> p) { return tree.rotateL(p); }
  protected Position<Entry<K,V>> rotateLR(Position<Entry<K,V>> p) { return tree.rotateLR(p); }
  protected Position<Entry<K,V>> rotateRL(Position<Entry<K,V>> p) { return tree.rotateRL(p); }
  ...
  protected void rebalanceInsert(Position<Entry<K,V>> p) { 
	  if(tree.getAux(p)==0)
		  tree.setAux(p, 1);
	  if(parent(p)!=null){
		  tree.setAux(parent(p), tree.getAux(parent(p))+1);
		  rebalanceInsert(parent(p));
	  }
  }
  ...
  protected void rebalanceDelete(Position<Entry<K,V>> p) { 
	  if(isRoot(p) && tree.getAux(p)==1)
		  tree.setAux(p, 0);
	  if(parent(p)!=null){
		  tree.setAux(parent(p), tree.getAux(parent(p))-1);
		  rebalanceDelete(parent(p));
	  }
  }
  ...
  private void displayAux(Position<Entry<K,V>> p, int depth){
	  String s="";
	  for(int i=0;i<depth;i++)
		  s+="\t";
	  if(!isExternal(p)){
		  System.out.println(s+p.getElement()+"	size: "+tree.getAux(p));
		  displayAux(left(p),depth+1);
		  displayAux(right(p),depth+1);
	  }
  }
  
  public int countLessThan(K k){
	  int count=countLessThanAux(k,0);
	  if(floorEntry(k).getKey()==k)
		  count++;
	  return count;
  }
  
  private int countLessThanAux(K k,int count){
	  if(inOrderPredecessor(k)==null)
		  return count;
	  count++;
	  return countLessThanAux(inOrderPredecessor(k),count);  
  }
  
  public K inOrderPredecessor(K k){
	  Entry<K,V> en=lowerEntry(k);
      if(en==null)
          return null;
      return en.getKey();
  }
  
  public K inOrderSuccessor(K k){
	  Entry<K,V> en=higherEntry(k);
      if(en==null)
          return null;
      return en.getKey();
  }
  
  public K findKthLargest(K k){
	  return findKthLargestAux(root(),(Integer) k);
  }
  
  private K findKthLargestAux(Position<Entry<K,V>> p,int k){
	  if(tree.getAux(right(p))+1==(int)k)
		  return p.getElement().getKey();
	  if(tree.getAux(right(p))+1>(int)k)
		  return findKthLargestAux(right(p),k);
	  return findKthLargestAux(left(p),(int)k-(tree.getAux(right(p))+1));
  }
}